...
export class AuthService {
	...
	async getUserAuthentication(username: string, password: string) {
		const user001mb: User001mb = await this.loginRepository.findOne({relations: ["person"], where: { username: username } });
		...
		if (user001mb) {
			...
			if (isMatch) {
				...
				return this.generateJwt(user001mb.username, user001mb.status, user001mb.securityquestion, user001mb.securityanswer).pipe(map((jwt: string) => {
