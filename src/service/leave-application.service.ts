...
var html = fs.readFileSync("leaveApplication.html", "utf8");

@Injectable()
export class LeaveApplicationService {
	constructor(@InjectRepository(Leaveapp001mb) private readonly leaveApplicationRepository: Repository<Leaveapp001mb>) { }
	...
	async update(leaveappDTO: LeaveappDTO): Promise<Leaveapp001mb> {
		const leaveapp001mb = new Leaveapp001mb();
		leaveapp001mb.setProperties(leaveappDTO);
		await this.leaveApplicationRepository.update({ lvappId: leaveapp001mb.lvappId }, leaveapp001mb);
		...
		if (leaveApplication.length < 0) {
			...
			pdf.create(document, options).then((pathRes) => {
				...
				response.writeHead(200, {
					"Content-Disposition": "attachment;filename=" + "leaveApplication.pdf",
	...
	async downloadExcel(@Req() request: Request, @Res() response: Response) {
		...
		if (leaveApplication.length < 0) {
			...
			{ key: 'I', width: 15.0 }, { key: 'J', width: 20.0 }, { key: 'K', width: 20.0 }];
